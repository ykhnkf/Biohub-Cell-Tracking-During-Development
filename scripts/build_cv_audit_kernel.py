...
# Official baseline/evaluator package is public and pip-installable from the checked-out source.
# Kaggle notebook internet is enabled for this audit kernel; final submission kernels will remain offline.
subprocess.run(["git", "clone", "--depth", "1", "https://github.com/royerlab/kaggle-cell-tracking-competition.git", "/kaggle/working/official"], check=True)
subprocess.run([sys.executable, "-m", "pip", "install", "-q", "-e", "/kaggle/working/official"], check=True)
sys.path.insert(0, "/kaggle/working/official/scripts")
sys.path.insert(0, "/kaggle/working/official/src")
...
if not geffs:
    # Some Kaggle layouts may place .geff next to .zarr under subdirectories.
...
for path in geffs:
    ...
    # GT -> GT should be a near-perfect metric self-test and catches format/version issues.
...
# Leave-one-dataset-out folds are the safest primary split when the number of datasets is modest.
...
# Also build a deterministic greedy 5-fold split balancing edge counts first and divisions second.
k = min(5, len(rows))
...
def main() -> None:
    ...
    print(json.dumps({"kernel": KERNEL_ID, "out": str(out)}, indent=2))
